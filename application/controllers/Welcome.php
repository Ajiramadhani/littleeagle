...
class Welcome extends CI_Controller {
	...
	{		
		// 4 artikel terbaru
		$data['artikel'] = $this->db->query("SELECT * FROM artikel,pengguna,kategori WHERE artikel_status='publish' AND artikel_author=pengguna_id AND artikel_kategori=kategori_id ORDER BY artikel_id DESC LIMIT 4")->result();
		
 
		// 4 course terbaru
		$data['event'] = $this->db->query("SELECT * FROM event,pengguna,kategori WHERE event_status='publish' AND event_author=pengguna_id AND event_kategori=kategori_id ORDER BY event_id DESC LIMIT 8")->result();
		$data['mapel'] = $this->db->query("SELECT * FROM event,pengguna,kategori WHERE event_status='publish' AND event_author=pengguna_id AND event_kategori=kategori_id ORDER BY event_id DESC LIMIT 3")->result();
	...
	public function det_guru($slug)
	{
		...
		$data['popular'] = $this->db->query("SELECT * FROM event,pengguna,kategori WHERE event_status='publish' AND event_author=pengguna_id AND event_kategori=kategori_id ORDER BY event_id DESC LIMIT 8")->result();
		...
		if(count($data['event']) > 0){
			...
			$data['meta_description'] = substr($data['event'][0]->event_konten, 0,200);
		}else{
			$data['meta_keyword'] = $data['pengaturan']->nama;
			$data['meta_description'] = $data['pengaturan']->deskripsi;
		}
	...
	{		
		...
		// $this->load->view('frontend/v_header',$data);
		$this->load->view('frontend/v_course', $data);
		// $this->load->view('frontend/v_footer',$data);
	}
